...
#设置工作路径
wkdir <- "D:\\HerbAgent\\Random_Walk\\result" #这里需要改成自己的文件夹地址
setwd(wkdir)
...
Formula_ingredient  <- read.csv("Formula_1_herb_ingredient.csv")
...
Ingredient_target  <- read.csv("Formula_1_ingredient_target.csv")
...
# 读取Syndrome文件  
Syndrome <- read.csv("Syndrome_1.csv", header = TRUE)  
...
write.csv(Score, file = "Formula_1_drug_sort_1seed---w.csv", row.names = FALSE)
write.csv(Score2, file = "Formula_1_GlobaResult_1seed---w.csv", row.names = FALSE)
